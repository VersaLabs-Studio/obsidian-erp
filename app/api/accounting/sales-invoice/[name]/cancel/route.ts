// app/api/accounting/sales-invoice/[name]/cancel/route.ts
// 5.1-A3 — Implicit cascade cancel: submitted Payment Entries referencing
// this invoice are cancelled first (via lib/erp/cascade-cancel), then the
// invoice itself. Mirrors the PI cancel route (4.1-C3).
...
export async function POST(
  ...
  try {
    const cancelled = await cascadeCancel(client, "Sales Invoice", docName);
    return NextResponse.json({
      success: true,
      data: { cancelled },
      message: `Sales Invoice ${docName} cancelled` +
        (cancelled.length > 1
